...
export async function POST(request: NextRequest) {
  ...
  const validPriorities = new Set<DbTaskPriority>(['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'])
  const tasks = Array.isArray(body.tasks) ? body.tasks.slice(0, 12).filter((task: unknown): task is Record<string, unknown> => typeof task === 'object' && task !== null).map(task => ({
    ...
    priority: typeof task.priority === 'string' && validPriorities.has(task.priority as DbTaskPriority) ? task.priority as DbTaskPriority : 'MEDIUM' as DbTaskPriority,
