...
export async function POST(request: NextRequest) {
  ...
  const project = await db.$transaction(async (transaction: Prisma.TransactionClient) => {
    ...
    const created = await transaction.project.create({
      data: { id: projectId, name: projectName, code: typeof body.code === 'string' ? body.code : undefined, description: typeof body.description === 'string' ? body.description : undefined, startDate: new Date(projectStartDate), targetDate: new Date(projectTargetDate), status: body.status === 'ON_HOLD' || body.status === 'COMPLETED' ? body.status : 'ACTIVE', userMembers: { create: { userId: user.id, role: 'PROJECT_ADMIN' } }, tasks: { create: tasks.map(task => ({ ...task, teamId: 'general', status: 'TODO', progress: 0, assignees: { create: { resourceId: task.ownerId } } })) }, events: { create: isAiDraft ? { type: 'ai.project_draft_confirmed', message: `AI draft confirmed by ${user.displayName}`, color: '#7C3AED', payload: { taskCount: tasks.length, source: 'project-draft', auditId } } : undefined } },
      include,
    })
