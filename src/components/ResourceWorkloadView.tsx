...
function workingDaysBetween(start: string, end: string, workingDays: number[]) {
  let total = 0
  const cursor = new Date(`${start}T00:00:00`)
  const last = new Date(`${end}T00:00:00`)
  while (cursor <= last) {
    if (workingDays.includes(cursor.getDay())) total += 1
...
export function ResourceWorkloadView() {
  ...
  const summaries = projectResources.map(resource => {
    ...
    const plannedHours = assignedActiveTasks.reduce((sum, task) => sum + (task.estimatedHours ?? priorityHours[task.priority]), 0)
    const dailyLoad = assignedActiveTasks.reduce((sum, task) => {
      const hours = task.estimatedHours ?? priorityHours[task.priority]
      const duration = task.plannedStartDate && task.plannedEndDate ? workingDaysBetween(task.plannedStartDate, task.plannedEndDate, workingDays) : 5
      return sum + hours / duration
    }, 0)
