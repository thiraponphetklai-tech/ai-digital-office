...
export interface ProjectIntelligence {
  ...
  wbsFollowUpTasks: { title: string; ownerId: string; dueDate?: string; blocker?: string }[]
...
export function getProjectIntelligence(project: Project, tasks: Task[], now = new Date()): ProjectIntelligence {
  ...
  const wbsFollowUpTasks = tasks.filter(task => {
    if (task.status === 'DONE') return false
    const dueDate = task.dueDate ? new Date(`${task.dueDate}T00:00:00`) : null
    return Boolean(task.blocker?.trim()) || Boolean(dueDate && dueDate <= in14Days)
  }).map(task => ({ title: task.title, ownerId: task.ownerId, dueDate: task.dueDate, blocker: task.blocker }))
...
export function formatIntelligenceLines(project: Project, intelligence: ProjectIntelligence, ownerNames: Record<string, string> = {}, includeWbsProgress = true) {
  return [
    ...
    ...(intelligence.wbsFollowUpTasks.length ? [`🔎 WBS tasks to follow up: ${intelligence.wbsFollowUpTasks.slice(0, 3).map(item => `${item.title} — Owner: ${ownerNames[item.ownerId] ?? item.ownerId}${item.dueDate ? `, due ${item.dueDate}` : ''}${item.blocker ? `, blocker: ${item.blocker}` : ''}`).join(' | ')}`] : []),
